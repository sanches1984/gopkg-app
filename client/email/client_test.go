...
func TestSendEmail(t *testing.T) {
	...
	client, _, err := NewClient(
		provider.NewSendGridProvider(os.Getenv("EMAIL_SEND_GRID_KEY")),
...
func testClient(t *testing.T, to []string, email *Client) {
	...
	t.Run("Send invite", func(t *testing.T) {
		err := email.Send(context.Background(), NewMessage("Тема письма с событием").
			WithTo(to...).WithCalendarCard(&provider.CalendarCard{
			Name:        "Супервстреча",
	...
	t.Run("Send invite all day", func(t *testing.T) {
		...
		err := email.Send(context.Background(), NewMessage("Тема письма на весь день").
			WithTo(to...).WithCalendarCard(&provider.CalendarCard{
...
<button style="background: #060; color: #fff; border-style: none; padding: 1em 0; text-align: center; display: block; width: 100%;" 
		...
		err := email.Send(context.Background(), NewMessage("Письмо с формой").
			WithTo(to...).WithHTML(html))
		assert.Nil(t, err)
	})
}
